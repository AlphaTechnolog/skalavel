import { IData, IDataRules, IValidationErrors } from "./interfaces";
...
export class Validator {
  ...
  public validate(): void {
    for (const [ruleKey, rule] of Object.entries(this.rules)) {
      ...
      if (
        this.data[ruleKey] === undefined &&
        splittedRules.indexOf("required") !== -1
      ) {
        ...
        if (stringRule !== "required" && stringRule !== "nullable") {
          if (this.data[ruleKey] !== undefined) {
            ...
            try {
              ...
              throw new Error(
                `[VALIDATOR]: ${splittedRule[0]} validator not found`
              );
            }
            let argv: string[] = [];
            ...
            if (!validated) {
              ...
              this.validationErrors[ruleKey].push(
                `${ruleKey} does not pass the test ${splittedRule[0]}`
              );
            }
          }
        }
